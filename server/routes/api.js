  ...
  try {
    ...
    timetables.forEach(tt => {
      ...
      daySchedule.periods.forEach(p => {
        ...
        const populateFac = (fac) => {
          if (!facultyBusyMap[fac]) facultyBusyMap[fac] = [];
          facultyBusyMap[fac].push(p.period);
        };

        if (Array.isArray(p.electiveAllocations)) {
          p.electiveAllocations.forEach(alloc => {
            if (alloc.faculty && alloc.faculty !== 'Multiple') populateFac(alloc.faculty);
            if (p.period === periodNum) {
              if (alloc.faculty) occupiedFaculty.add(alloc.faculty);
              if (alloc.room) occupiedRooms.add(alloc.room);
            }
          });
        } else {
          if (p.faculty && p.faculty !== 'Multiple') populateFac(p.faculty);
          if (p.period === periodNum) {
            if (p.faculty && p.faculty !== 'Multiple') occupiedFaculty.add(p.faculty);
            if (p.room && p.room !== 'Multiple') occupiedRooms.add(p.room);
          }
        }
      });
    });
    ...
    res.json({
      occupiedFaculty: Array.from(occupiedFaculty),
...
router.post('/generate', async (req, res) => {
  try {
    ...
    const targetConfigs = batchConfigs || { [batchId]: subjectConfig };
